...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext.prototype.login = function () {
        ...
        if (this.popUp) {
            ...
            return new Promise(function (resolve, reject) {
                that._loginPopup(urlNavigate).then(function (idtoken) {
                    resolve(idtoken);
                }, function (error) {
                    reject(error);
                });
        ...
        return new Promise(function (resolve, reject) {
            try {
                ...
                */
                ...
                this._loginInProgress = false;
                reject('Error opening popup' + e.message);
            }
        });
    }
    ...
    AuthenticationContext.prototype._loginPopup = function (urlNavigate) {
        var that = this;
        return new Promise(function (resolve, reject) {
            that._openPopup(urlNavigate, "login", that.CONSTANTS.POPUP_WIDTH, that.CONSTANTS.POPUP_HEIGHT).then(function (popupWindow) {
                if (popupWindow == null) {
                    that.warn('Popup Window is null. This can happen if you are using IE');
                    that._saveItem(that.CONSTANTS.STORAGE.ERROR, 'Error opening popup');
                    that._saveItem(that.CONSTANTS.STORAGE.ERROR_DESCRIPTION, 'Popup Window is null. This can happen if you are using IE');
                    that._saveItem(that.CONSTANTS.STORAGE.LOGIN_ERROR, 'Popup Window is null. This can happen if you are using IE');
                    if (that.callback)
                        that.callback(that._getItem(this.CONSTANTS.STORAGE.LOGIN_ERROR), null);
                    reject(that._getItem(that.CONSTANTS.STORAGE.LOGIN_ERROR));
                }
                if (that.config.redirectUri.indexOf('#') != -1)
                    var registeredRedirectUri = that.config.redirectUri.split("#")[0];
                else
                    var registeredRedirectUri = that.config.redirectUri;

                var pollTimer = window.setInterval(function () {
                    if (!popupWindow || popupWindow.closed || popupWindow.closed === undefined) {
                        that._loginInProgress = false;
                        window.clearInterval(pollTimer);
                        reject('popup window closed');
                    }
                    try {
                        if (popupWindow.location.href.indexOf(registeredRedirectUri) != -1) {
                            if (that.isAngular) {
                                ...
                                that.handleWindowCallback(popupWindow.location.hash).then(function (idtoken) {
                                    resolve(idtoken);
                                }, function (error) {
                                    reject(error);
                                });
    ...
    AuthenticationContext.prototype.handleWindowCallback = function (hash) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (that.isCallback(hash)) {
                var requestInfo = that.getRequestInfo(hash);
                that.info('Returned from redirect url');
                that.saveTokenFromHash(requestInfo);
                var callback = null;
                if ((requestInfo.requestType === that.REQUEST_TYPE.RENEW_TOKEN) && window.parent && (window.parent !== window)) {
                    // iframe call but same single page
                    that.verbose('Window is in iframe');
                    callback = window.parent.callBackMappedToRenewStates[requestInfo.stateResponse];
                    if (callback)
                        callback(that._getItem(that.CONSTANTS.STORAGE.ERROR_DESCRIPTION), requestInfo.parameters[that.CONSTANTS.ACCESS_TOKEN] || requestInfo.parameters[that.CONSTANTS.ID_TOKEN]);
                    return;
                } else if (requestInfo.requestType === that.REQUEST_TYPE.LOGIN) {
                    callback = that.callback;
                    if (requestInfo.parameters[that.CONSTANTS.ID_TOKEN] != '' && that._getItem(that.CONSTANTS.STORAGE.ERROR_DESCRIPTION) == '')
                        resolve(requestInfo.parameters[that.CONSTANTS.ID_TOKEN]);
                    else if (requestInfo.parameters[that.CONSTANTS.ID_TOKEN] == '' && that._getItem(that.CONSTANTS.STORAGE.ERROR_DESCRIPTION) != '')
                        reject(that._getItem(that.CONSTANTS.STORAGE.ERROR_DESCRIPTION));
                    if (callback)
                        callback(that._getItem(that.CONSTANTS.STORAGE.ERROR_DESCRIPTION), requestInfo.parameters[that.CONSTANTS.ID_TOKEN]);
                }
                if (!that.popUp)// No need to redirect user in case of popup
                    window.location = that._getItem(that.CONSTANTS.STORAGE.LOGIN_REQUEST);
            }
            else
                reject('window.location.hash does not contain id token or error');
        });
    };
